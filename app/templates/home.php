      ...
      <div class="results">
        <?php
          if(isset($_POST['submit_select_category'])){
            ...
            $query = "select title, avg(stars), summary, cover_image, total_count from book natural join ratings natural join category join (select count(*) as total_count from book natural join category where category = '". $category ."') as count_table where category = '". $category ."' group by isbn";
            $result = mysqli_query($conn, $query);

            $tr = mysqli_fetch_row($result);
            echo '<div class="inner">showing '.$tr[4].' results filtered by category: '. $category .'</div><br><div class="inner">';

            while($tr){
              echo generateBookItem($tr[0], "data:image/jpeg;base64," . blobToSrc($tr[3]), $tr[1]*20, $tr[2]);
              $tr = mysqli_fetch_row($result);
            }
          ...
          if(isset($_POST['submit_sortby_rating'])){
            ...
            $query = "select title, avg(stars) as rating, summary, cover_image, total_count from book natural join ratings join (select count(*) as total_count from book) as count_table group by isbn order by rating desc";
            $result = mysqli_query($conn, $query);
          ...
          if(isset($_POST['submit_sortby_pop'])){
            ...
            $query = $query = "SELECT b.title, avg(stars), b.summary, b.cover_image, total_count, total_borrowed_tb.total_borrowed as tot_bor
          from book as b natural join ratings
          join (
          ...
          if(isset($_POST['submit_search'])){
            ...
            $query1 = "select isbn, title, avg(stars) as rating, summary, cover_image from book natural join ratings where title like '%".$terms."%' group by isbn";
            $query2 = "select isbn, title, avg(stars) as rating, summary, cover_image from book natural join ratings where isbn like '%".$terms."%' group by isbn";
            $query3 = "select isbn, title, avg(stars) as rating, summary, cover_image from book natural join ratings natural join author where name like '%".$terms."%' group by isbn";
            $query = "select distinct merged.title, merged.rating, summary, cover_image from ((".$query1.") UNION (".$query2.") UNION (".$query3.")) as merged";
            $result = mysqli_query($conn, $query);
