<form class="details-card"action=""method="post">
  <div class="details-control">
    ...
    if(isset($_POST['submit-new-details'])){
      if($is_valid_handler || ($is_teacher && $is_my_profile)){
        $new_sch_id = $_POST['sch-id'];
        $new_name = $_POST['name'];
        $new_email = $_POST['email'];
        $new_pass = $_POST['password'];

        if($name == $new_name && $sch_id == $new_sch_id && $email == $new_email && $password == $new_pass){
        ...
        if($password == $new_pass){
          ...
          $query = "update user set password = '".$new_pass."' where username = '".$page_username."'";
          $result = mysqli_query($conn, $query);
          if($result){
            echo "<label class='feedback green'>changes saved! Refreshing page </label>";
            echo '<script>window.location.href = window.location.href;</script>';
  ...
  <div class="details-card-row">
    ...
    <div class="details-card-right">
      ...
      <?php
      if($is_valid_handler || ($is_teacher && $is_my_profile)) {
        echo '<div class="details-right-input"><input type="text"name="name"value="'.$name.'" required=""></div>';
      }
      ?>
  ...
  <div class="details-card-row">
    ...
    <div class="details-card-right">
      ...
      <?php
      if($is_valid_handler || ($is_teacher && $is_my_profile)) {
        echo '<div class="details-right-input"><input type="text"name="email"value="'.$email.'" required=""></div>';
  ...
  <div class="details-card-row">
    ...
    <div class="details-card-right">
      ...
      <?php
      if($is_valid_handler || ($is_teacher && $is_my_profile)) {
        echo '<div class="details-right-input">
...
<form action=""method="post">
  ...
  <?php
  if($is_valid_handler || $is_my_profile){
    echo '<button class="button" name="submit_deactivate_account" type="submit">
      <span class="button_lg">
        <span class="button_sl"></span>
        <span class="button_text">DEACTIVATE ACCOUNT</span>
      </span>
    </button>';
  }
  ?>
  ...
  if(isset($_POST['submit_deactivate_account'])){
    $query = "update user set user_verified = 0 where username = '".$page_username."'";
    $result = mysqli_query($conn, $query);
    if($result){
      echo "<label class='feedback green'>account deactivated!</label>";
    }
    else{
      echo "<label class='feedback red'>[database error] failed, try again.</label>";
    }
