...
maximum_n_of_copies_per_sch = 20
...
for i in range(number_of_users):
	...
	row.append(u+str('@bookbyte.com'))
	row.append(fake.name())
	row.append(str(sch_id))
	
	ver = "0"
	if random.randint(1,100) < 85: #prob of being verified
		counter2 += 1
		ver = "1"
...
with open('user.csv', 'w', newline='') as csvfile:
	writer = csv.writer(csvfile)
	writer.writerows(data)
printres("verified users",counter2,40)
n_users = counter2
# ==========================
#	teacher.csv
# ==========================
...
printres("verified handlers",counter2,1)
n_teachers = counter
...
printres("students (min)",n_users - n_teachers - counter,30)
...
for j in isbns:
	...
	for i in range(1,number_of_schools+1):
		if random.randint(1,100) < 60: #prob of having book
			counter += 1
			row = []
			row.append(i)
			row.append(j)
			copies = random.randint(3,maximum_n_of_copies_per_sch)
			counter2 += copies
...
for i in usernames:
	for j in isbns:
		if random.randint(1,100) < 15: #prob of having rated this book
			counter+=1
			row = []
			row.append(i)
			row.append(j)
...
for i in range(len(usernames)):
	...
	for j in isbns:
		...
		if random.randint(1,len(isbns)+70) < 3: #prob of having borrowed this book
			...
			row.append(str(random.randint(2022,2023)) + "-" + str(random.randint(1,2)).zfill(2) + "-" + str(random.randint(1,19)).zfill(2) + " " + str(random.randint(1,23)).zfill(2) + ":" + str(random.randint(1,59)).zfill(2)+ ":" + str(random.randint(1,59)).zfill(2))
			row.append(user_schools[rand_handler])
			row.append('borrowed') #in_out
			tr_ver = 0
			if random.randint(1,100) < 70: #prob of beight verified
				counter+=1
				tr_ver = 1
				if random.randint(1,100) < 85: #prob of been returned
					tr_ver = 2
			row.append(tr_ver)
			...
			data.append(row)
			# if borrowed, then:
			if tr_ver == 2: #prob of having returned it if borrowed
				loans -= 1
				counter2+=1
				rand_handler = random.choice(handler_usernames)
				row = []
				...
				row.append(j)
				row.append(rand_handler)
				row.append(str(random.randint(2023,2023)) + "-" + str(random.randint(2,2)).zfill(2) + "-" + str(random.randint(20,27)).zfill(2) + " " + str(random.randint(1,23)).zfill(2) + ":" + str(random.randint(1,59)).zfill(2)+ ":" + str(random.randint(1,59)).zfill(2)) #greater than borrowing date
				row.append(user_schools[rand_handler])
...
with open('loan.csv', 'w', newline='') as csvfile:
	writer = csv.writer(csvfile)
	writer.writerows(data)
printres("total verified borrows",counter,50)
...
for i in usernames:
	...
	for j in isbns:
		...
		if random.randint(1,len(isbns)+60) < 4: #prob of making this reservation
			...
			row.append(i)
			row.append(j)
			row.append(str(random.randint(2023,2023)) + "-" + str(random.randint(1,2)).zfill(2) + "-" + str(random.randint(1,27)).zfill(2) + " " + str(random.randint(1,23)).zfill(2) + ":" + str(random.randint(1,59)).zfill(2)+ ":" + str(random.randint(1,59)).zfill(2))
...
print("\n=== check if the above values are\n=== good (green), or (adjust parameters\n=== and probabilities and) run it\n=== again.")
